...
@endsection
@section('navbar_post')
    <form action="{{ route('admin.posts.destroy', ['post' => $post->id, 'permanent'=>true]) }}" method="post">
        @csrf
        @method('delete')
        <input type="submit" value="Permament delete" class="btn btn-danger">
    </form>
    <a class='btn btn-primary' href="{{route('admin.posts.indexTrashed')}}">Back</a>
@endsection
